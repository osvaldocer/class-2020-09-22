...
sep_old <- 
  ...
  summarize(u_grad_pop = sum(u_grad), .groups = "drop") %>%
  filter(u_grad_pop > 10)
View(sep_old)
...
sep_new <-
  ...
  select(-grad, -non_degree, -x_reg, -vus, -employee, 
         -withdraw, -total, -instructor_full_name, -course_section_code) %>%
  rename(id = course_id, title = course_title, name = course_name, department = course_department) %>%
  group_by(title, id, name, department) %>% 
  summarize(new_u_grad_pop = sum(u_grad), .groups = "drop") %>%
  filter(new_u_grad_pop > 10)
view(sep_new)
```
...
- keep in mind potentially missing data



After we have a model, Temperance suggests we should:
- keep in mind individual variation
- keep in mind uncertainty when measuring the ATE
- randomization is impossible in this situation
...
sep_both %>%
  mutate(changes = new_u_grad_pop - u_grad_pop) %>%
